# Handoff → Cursor Grok 4.5 (paste this whole file as the task prompt — retargeted from Codex, which is out of usage)

You are Grok 4.5, Cursor team lead. START A FRESH CHAT for this (long chats drift). Pull `main` into the `cursor` branch first; never work on `main`.
Mark cursor_grok_4_5 ACTIVE in `artifacts/agent_ledger.json`. Delegate Composer-sized pieces (data transcription) to Composer to save Grok credits; you review.
...
- Typecheck + production build green before every commit. Close out per
  `docs/ai-team/end-here.md` (session file, signed DEVLOG, merge cursor → main —
